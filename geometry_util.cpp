...
#include <cmath>
#include <stdexcept>
geo::CSG_Sphere::CSG_Sphere(Eigen::Vector3f center, float radius)
{
    this->center = center;
    this->radius = radius;
...
void geo::CSG_Sphere::getBoundingBox(float min[3], float max[3])
{
...
bool geo::CSG_Sphere::rayIntersectIntervals(Eigen::Vector3f &rayO, Eigen::Vector3f &rayD, DisjointIntervals &interior, geo::NormalsMap &normalsMap)
{
...
geo::CSG_Box::CSG_Box(Eigen::Vector3f &min, Eigen::Vector3f &max, Eigen::Transform<float, 3, Eigen::Affine> origin_transform)
{
    this->size_min = min;
    this->size_max = max;
...
void geo::CSG_Box::getBoundingBox(float min[3], float max[3])
{
    for(auto i : {0,1,2}){
...
bool geo::CSG_Box::rayIntersectIntervals(Eigen::Vector3f &rayO, Eigen::Vector3f &rayD, DisjointIntervals &interior, geo::NormalsMap &normalsMap)
{
...
geo::CSG_Cylinder::CSG_Cylinder(Eigen::Vector3f center, Eigen::Vector3f direction, float radius, float height_min, float height_max):center(center),direction(direction.normalized()),radius(radius), height_min(height_min), height_max(height_max)
{
...
void geo::CSG_Cylinder::getBoundingBox(float min[3], float max[3])
{
...
bool geo::CSG_Cylinder::rayIntersectIntervals(Eigen::Vector3f &rayO, Eigen::Vector3f &rayD, DisjointIntervals &interior, geo::NormalsMap &normalsMap)
{
    ...
    if(std::abs(rayD.dot(direction)) < 1e-5){
        if(a.dot(direction) > 0.0 && b.dot(direction) < 0.0){
...
bool geo::CSG_Union::rayIntersectIntervals(Eigen::Vector3f &rayO, Eigen::Vector3f &rayD, DisjointIntervals &interior, geo::NormalsMap &normalsMap)
{
    for(auto obj : objects){
        geo::NormalsMap normalsMap_;
        DisjointIntervals interior_;
        obj->rayIntersectIntervals(rayO, rayD, interior_, normalsMap_);
        normalsMap.insert(normalsMap_.begin(), normalsMap_.end());
        interior.unionWith(interior_);
    }
}

void geo::CSG_Union::getBoundingBox(float min[3], float max[3])
{
...
geo::CSG_Union::CSG_Union(std::vector<std::shared_ptr<geo::CSG> > &objects)
    :objects(objects)
{

}

bool geo::CSG::rayIntersect(Eigen::Vector3f &rayO, Eigen::Vector3f &rayD, float &t, Eigen::Vector3f &normal)
{
    DisjointIntervals interior;
    geo::NormalsMap normalMaps;
    ...
    if(t==0.0) throw std::logic_error("t should be positive here!");
    auto it = normalMaps.find(t);
